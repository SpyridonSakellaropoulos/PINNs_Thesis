%Πεπερασμένες Διαφορές με RK4 για την ΜΔΕ Κύματος με Dirichelt
...
u0=A*cos(pi/L*x).'; %διάνυσμα στήλης
...
writematrix(u, 'u_wave_n.csv');
%writematrix(v, 'v_wave_d.csv');
%writematrix(x', 'x_wave_d.csv');
%writematrix(t', 't_wave_d.csv');
...
ylabel('t');
zlabel('u(x,t)');
title('Λύση της Εξίσωσης Κύματος (3D Surface)');
colorbar;
...
function dw=f(w,t,dx,c)
    ...
    du=[v(1:N)]; %αποτέλεσμα υπολογισμού πρώτου μισού στήλης
